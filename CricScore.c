#include<stdio.h>
#include<stdlib.h>

struct batsman{
    char name[25];
    int runs,score,balls,toruns,tobal,ones,twos,threes,fours,sixes;
    int max_six,max_runs,max_four;
    float str;
}pl1[100],pl3;

struct bowler{
    char name[25];
    int runsgv,wkttkn,overs;
    int max_x;
    float econ;
}pl2[100],pl4;

int main(){
    int plno,choice;
    int i,n,m;
    printf("enter the batsman details:\n");
    printf("enter the number of batsman:\n");
    scanf("%d",&n);
    for(i=0;i<m;i++){
        printf("enter name off batsman %d:\n",i+1);
        scanf("%s",&pl1[i].name);
        printf("enter the number of ones scored by the player %d:\n",i+1);
        scanf("%d",&pl1[i].ones);
         printf("enter the number of twos scored by the player %d:\n",i+1);
        scanf("%d",&pl1[i].twos);
         printf("enter the number of threes scored by the player %d:\n",i+1);
        scanf("%d",&pl1[i].threes);
         printf("enter the number of fours scored by the player %d:\n",i+1);
        scanf("%d",&pl1[i].fours);
         printf("enter the number of sixes scored by the player %d:\n",i+1);
        scanf("%d",&pl1[i].sixes);

        printf("enter the balls played by the player%d:\n",i+1);
        scanf("%d",&pl1[i].balls);
    }

    for(i=0;i<n;i++){
        printf("\nenter the name of the bowler%d:",i+1);
        scanf("%d",&pl2[i].name);

        printf("\nenter the runs given by the bowler%d:",i+1);
        scanf("%d",&pl2[i].runsgv);

        printf("\nenter the overs bowled by the bowler%d:",i+1);
        scanf("%d",&pl2[i].overs);

        printf("\nenter the wickets taken by the bowler%d:",i+1);
        scanf("%d",&pl2[i].wkttkn);


    }
    printf("Thank you all the details are recorded\n");

    do{
        printf("enter the choice:\n 1)batsman details:\n 2)bowler details:\n 3)match summary:\n 4)record:\n 5)exit:\n");
        scanf("%d",&choice);
        switch (choice)
        {
        case 1:
            printf("enter the batsman number to see his details\n");
            scanf("%d",&plno);
            plno--;
            printf("               Player details\n");
            printf("============================\n");
            printf(" batsman       runs     ballls    fours    sixes     SR    \n");
            printf("============================\n ");
            pl1[plno].runs=(1*pl1[plno].ones) + (2*pl1[plno].twos) + (3*pl1[plno].threes) + (4*pl1[plno].fours) + (6*pl1[plno].sixes) ;
            pl1[plno].str=(pl1[plno].runs* 100.00/pl1[plno].balls);
            printf("%-15s %-14d %-13d %-11d %-11d %-9.2f\n\n",pl1[plno].name,pl1[plno].runs,pl1[plno].balls,pl1[plno].fours,pl1[plno].sixes,pl1[plno].str);
            break;
        case 2:
        printf("enter the bowler number to see his details\n");
        scanf("%d",&plno);
        plno--;
         printf("               Player details\n");
            printf("============================\n");
            printf(" bowlers      overs     runs    wickets    economy     \n");
            printf("============================\n ");
            for(i=0;i<n;i++){
                pl2[plno].econ=pl2[plno].runsgv/pl2[plno].overs;
                printf("%-15s %-14d %-13d %-11d %-11.2f\n\n",pl2[plno].name,pl2[plno].overs,pl2[plno].runsgv,pl2[plno].wkttkn,pl2[plno].econ);
            }
            
            break;

        case 3:
        printf("               match summary\n");
            printf("============================\n");
            printf(" batsman       runs     ballls    fours    sixes     SR    \n");
            printf("============================\n ");
            for(i=0;i<n;i++){
                pl1[plno].runs=(1*pl1[plno].ones) + (2*pl1[plno].twos) + (3*pl1[plno].threes) + (4*pl1[plno].fours) + (6*pl1[plno].sixes) ;
            pl1[plno].str=(pl1[plno].runs* 100.00/pl1[plno].balls);
            printf("%-15s %-14d %-13d %-11d %-11d %-9.2f\n\n",pl1[plno].name,pl1[plno].runs,pl1[plno].balls,pl1[plno].fours,pl1[plno].sixes,pl1[plno].str);

            }
            printf("TOTAL RUNS:%d\n\n",pl3.toruns);
            printf("\n\n");
            printf("============================\n");
            printf(" bowlers      overs     runs    wickets    economy     \n");
            printf("============================\n ");
            for(i=0;i<n;i++){
              pl2[plno].econ=pl2[plno].runsgv/pl2[plno].overs;
                printf("%-15s %-14d %-13d %-11d %-11.2f\n\n",pl2[plno].name,pl2[plno].overs,pl2[plno].runsgv,pl2[plno].wkttkn,pl2[plno].econ);  
            }
            
            break;
        case 4:
            pl3.max_runs=0,pl4.max_x=0,pl3.max_four=0,pl3.max_six=0;
            for(i=0;i<m;i++){
                pl1[i].runs=(1*pl1[i].ones)+(2*pl1[i].twos) + (3*pl1[i].threes) + (4*pl1[i].fours) + (6*pl1[i].sixes) ;
                if(pl3.max_runs<pl1[i].runs){
                    pl3.max_runs=pl1[i].runs;
                }
                if(pl3.max_six<pl1[i].sixes){
                    pl3.max_six=pl1[i].sixes;
                }
                if(pl3.max_four<pl1[i].fours){
                    pl3.max_four=pl1[i].fours;
                }
                if(pl4.max_x<pl2[i].wkttkn){
                    pl4.max_x=pl2[i].wkttkn;
                }
            }
            printf("highest runs scored by the batsman:%d\n",pl3.max_runs);
            printf("highest fours scored by the batsman:%d\n",pl3.max_four);
            printf("highest sixes scored by the batsman:%d\n",pl3.max_six);
            printf("highest wickets taken by the bowler:%d\n",pl4.max_x);
            

            break;
        case 5:
        exit(1);

        default:
        printf("enter the correct choice\n");
            break;
        }
    }while(choice!=5);
}